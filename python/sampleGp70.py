...
def init_GP70():   
    ...
    GPIO.setup(27,   GPIO.OUT, initial=GPIO.HIGH )        # RPi-GP70絶縁電源ON
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
                prog='sampleGp70.py',          #プログラムファイル名
                usage='RPi-GP70のサンプルプログラム', #使用方法
                description='引数なし',
                epilog=     '--------------------------------------------------------------------------',
                add_help=True,
                )
    ...
    try:
        ...
        while( key != 0 ):
            i = input( "1:送受信テスト(RS422全二重) 2:1:送受信テスト(RS485半二重) 3:設定 0:終了 > " )
            ...
            if( (key == 1)or(key == 2) ):         # '1'または'2'なら、送受信テスト
                # [RS422全二重モードの設定]
                # 基板上のジャンパを、[RS-422の全二重で受信両端の終端抵抗をON]に設定する。
                ...
                if( key == 1 ):   # '1'なら、全二重の設定
                    # RPi-GP70のRS422では、TXDENにRTS信号を使用。全二重で通信するにはRTSを常にFalse状態とする。
                    if(s0.rtscts==False):
                        s0.rts=False
                    if(s1.rtscts==False):
                        s1.rts=False
                ...
                if( key == 2 ):   # '2'なら、RS485モードの設定
                    # RPi-GP70ではRTSの設定を、(rts_level_for_tx=False, rts_level_for_rx=True)とする。
                    # ループバックは(loopback=False)でも、半二重設定だと送信データはエコーバックされる。
                    # RPi-GP70のRS485ドライバはハードウェアでネイティブに制御されるので、(delay_before_tx=None, delay_before_rx=None)とする。
